...
public class HistoryScene extends Page implements EventHandler<ActionEvent>{
	...
	public HistoryScene(User userData) {
	...
	private void setTransactionHeaderData() {
		...
		String query = String.format("SELECT th.userID, mu.UserEmail, th.TransactionID, th.TransactionDate FROM transactionheader AS th JOIN msuser AS mu ON th.UserID = mu.UserID WHERE th.UserID = '%s'", userId);
